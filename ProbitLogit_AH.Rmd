...
Authors: "Andrea, Hannah, and Yifan"
output:
  html_document:
    df_print: paged
---
...
#Rename Race 
mld$race <- ifelse((mld$black == 0) & (mld$hispanic == 0), "White", 
                   ifelse((mld$black == 1) & (mld$hispanic == 0), "Black", "Hispanic"))
...
```{r}
# Only select individuals who met guideline
sub1 <- mld %>%
  ...
  facet_wrap(~race) +
  theme_classic()+
  ggtitle("Percentage of approval if guidelines are met")
  
sub1
```

+ If applicants met the guidelines, black and hispanic applicants still have lower approval rate than white applicants.

+ For Hispanic candidates, married applicants have higher declined rate than unmarried if guidelines are met. 

+ For white and black candidates, if guidelines are met, unmarried applicants have higher decliend rate.

```{r}
# Only select individuals who didn't meet guideline
sub2 <- mld %>%
  filter(guideline == "No") %>%
  group_by(marital, race, approve) %>%
  summarise(count=n()) %>%
  ...
  facet_wrap(~race) +
  theme_classic()+
  ggtitle("Percentage of approval if guidelines are not met")
  
sub2

```

+ If guidelines are not met, white married applicants have highest approval rate.
...
mld_white <- filter(mld, race == "White")
mld_black <- filter(mld, race == "Black")
```
...
```{r}
probit_all <- glm(approve ~ race + guideline + obligation + loan + marital, family = binomial(link = "probit"), data = mld)
...
## Overall
```{r}
logit <- glm(approve ~ race + guideline + obligation + loan  + marital, data = mld, family = "binomial")
